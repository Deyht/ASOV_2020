...
int main()
{
	int nb_dat = 150, in_dim = 4, out_dim = 3, nb_epochs = 8000;
	float **input, **targ, *output, **weights;
	float h, learn_rate = 0.005, rdm, mean, maxval;
	...
	for(i = 0; i < in_dim+1; i++)
		for(j = 0; j < out_dim; j++)
			weights[i][j] = (rand()/(double)RAND_MAX)*(0.02)-0.01;

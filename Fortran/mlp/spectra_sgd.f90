...
program multi_layer_perceptron
	...
	integer, parameter :: nb_train = 1015, nb_test = 100, in_dim = 3753, hid_dim = 8
	integer, parameter :: out_dim = 7, nb_iter = 100, control_interv = 5
	...
	do t = 1, nb_iter
		...
		do i=1, nb_train
			...
			i_d  = int(rdm*nb_train) + 1
			
			call forward(input(i_d,:), in_dim, hidden, hid_dim, output, out_dim, weights1, weights2, beta)
			
			call backprop(input(i_d,:), in_dim, hidden, hid_dim, output, targ(i_d,:), out_dim, weights1, weights2, learn_rate, beta)
			
			! Compute error on training dataset
			quad_error = quad_error + 0.5*sum((output(:) - targ(i_d,:))**2)
		end do

...
program multi_layer_perceptron
	...
	integer, parameter :: nb_train = 568, nb_test = 200, in_dim = 8, hid_dim = 12, out_dim = 2, nb_epochs = 1500, control_interv = 10
	...
	real :: h, learn_rate = 0.05, beta = 1.0
	...
	do i = 1, nb_train
		if(input(i,1) > 6) input(i,1) = 6
		input(i,8) = int(mod(input(i,8)-30,10.0))
		if(input(i,8) > 5) input(i,8) = 5
	end do	

	do i = 1, nb_test
		if(input_test(i,1) > 6) input_test(i,1) = 6
		input_test(i,8) = int(mod(input_test(i,8)-30,10.0))
		if(input_test(i,8) > 5) input_test(i,8) = 5
	end do

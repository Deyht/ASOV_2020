...
int main()
{
	int nb_train = 1015, nb_test=100, in_dim = 3753, hid_dim = 8, out_dim = 7, nb_iter = 100, control_interv=10;
	...
	f = fopen("../../data/stellar_spectra/train.dat", "r+");
	if(f == NULL)
	{
	...
	f2 = fopen("../../data/stellar_spectra/target.dat", "r+");
	if(f2 == NULL)
	{
		printf("File not found\n");
		exit(EXIT_FAILURE);
	...
	for(t = 0; t < nb_iter; t++)
	{
		...
		for(i=0; i < nb_train; i++)
		{
			...
			//Forward phase
			forward(input[ind], in_dim, hidden, hid_dim, output, out_dim, weights1, weights2, beta);
			
			//Back-propagation phase
			backprop(input[ind], in_dim, hidden, hid_dim, output, targ[ind], out_dim, weights1, weights2, learn_rate, beta);
		
			for(j = 0; j < out_dim; j++) 
				quad_error += (output[j] - targ[ind][j])*(output[j] - targ[ind][j]); 
		}
